    ...
    fn recur(&mut self, type_env: &mut TypeEnv, node_id: &NodeId) -> Type /* Result<Type, NodeIdError> */
    {
        ...
        match data.node_type {
            ...
            Variable => {
                ...
                let scheme =
                    type_env.get_or_let_fresh(&String::from(root_name), &mut self.fresh_vars);
            ...
            Block => {
                ...
                let (in_types, out_type) = if children.len() > 1 {
                    ...
                    for (id, ty) in arg_node_ids.into_iter().zip(&mut self.fresh_vars) {
                        ...
                        local_env.explicitly_define(
                            String::from(self.tree.node_id_str(id).unwrap()),
                            ty,
                        );
                    }
            ...
            Apply => {
                ...
                let sig = match self.functions.get_signature(func_name) {
                    None => {
                        self.errors.push(
                            Problem::UnknownFunction.at(self
                                .tree
                                .nodes
                                .get(&children[0])
                                .unwrap()
